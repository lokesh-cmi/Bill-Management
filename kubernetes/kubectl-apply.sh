...
logSummary() {
    echo ""
        ...
        echo "ui - http://minikube_ip_placeholder:30200/"
...
default() {
    ...
    kubectl create configmap minikube-cm  --from-literal=minikube-ip=$(kubectl get node -o jsonpath='{.items[0].status.addresses[?(@.type=="InternalIP")].address}') -n billmanagement
    kubectl label namespace billmanagement istio-injection=enabled --overwrite=true
    ...
    kubectl apply -f ui-${suffix}/
    kubectl apply -f customermanagement-${suffix}/
    kubectl apply -f invoicemanagement-${suffix}/
    kubectl apply -f paymentmanagement-${suffix}/
}
