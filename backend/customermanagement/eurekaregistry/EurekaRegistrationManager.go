...
import (
	...
	"com.bill.customer/eurekaregistry/helper"
...
func (erm EurekaRegistrationManager) RegisterWithSerivceRegistry(eurekaConfigs RegistrationVariables) {
	...
	helper.MakePostCall(eurekaConfigs.ServiceRegistryURL+"customermanagement", body, nil)
	time.Sleep(10 * time.Second)
	bodyUP := erm.getBodyForEureka("UP", eurekaConfigs)
	helper.MakePostCall(eurekaConfigs.ServiceRegistryURL+"customermanagement", bodyUP, nil)
}

func (erm EurekaRegistrationManager) SendHeartBeat(eurekaConfigs RegistrationVariables) {
	logger.Infof("Sending HeartBeat")
	job := func() {
		helper.MakePutCall(eurekaConfigs.ServiceRegistryURL+"customermanagement/"+eurekaConfigs.InstanceId, nil, nil)
	}
...
func (erm EurekaRegistrationManager) DeRegisterFromServiceRegistry(configs RegistrationVariables) {
	...
	helper.MakePostCall(configs.ServiceRegistryURL+"customermanagement", bodyDOWN, nil)
}
